...
$\sim26$\,TFLOP/s, motivating quantization to reduce storage, memory traffic,
...
\begin{itemize}
  \item Adding INT8 post-\gls{ReLU} activation quantization to 8-bit
    weight-\gls{QAT} models degrades the required $E_b/N_0$ by at most
    $0.08$\,dB.
...
%Section~\ref{sec:sys_nrx} describes the uplink model and the GNN--CNN
%receiver. Section~\ref{sec:nrx_qat} presents weight and activation \gls{QAT}
%together with unstructured pruning and subsequent sections discuss results and performance analysis.
