...
Optimizer rows report the step count and learning rate for two
successive Adam phases.}
...
\begin{figure*}[t]
    ...
    \begin{subfigure}[t]{0.48\linewidth}
        ...
        \label{fig:bler_prune}
    \end{subfigure}
    \caption{\gls{BLER} versus $E_b/N_0$ on DoubleTDL-low for
    (a) W$b$A8 \gls{QAT} and (b) weight-\gls{QAT} checkpoints after
    50\% magnitude pruning. Curves show FP8~(E4M3), INT8,
    FP4~(E2M1), and INT4.}
    \label{fig:BLER}
\end{figure*}
...
\begin{figure*}[t]
    ...
    with the INT4 and FP4 grids. Panel~(b) groups prune-eligible
    pointwise and dense kernels by \gls{CGNN} stage. Its y-axis labels
...
Table~\ref{tab:sim_params}. We report \gls{BLER} versus rate-adjusted $E_b/N_0$ at the $10\%$ and
$1\%$ operating points. Baselines include the full-precision (\texttt{FP32}) neural receiver,
LS channel estimation with LMMSE equalization, LMMSE channel
...
most $0.08$\,dB at either target. Both receivers outperform LS--LMMSE
by approximately $1.83$--$1.91$\,dB and remain within
$\sim0.2$\,dB of LMMSE--K-Best. Thus, A8 causes no observed link
penalty in this setting. Section~\ref{sec:res_prune} evaluates pruning
from separate weight-only checkpoints.

\vspace{-2mm}

\subsection{FP4 versus INT4 Weight QAT}
\label{sec:res_fp4}
For weight-only QAT, the format strongly affects 4-bit performance
(Table~\ref{tab:wo_vs_a8}). FP4~(E2M1)
requires $1.32$\,dB and $1.40$\,dB more $E_b/N_0$ than \texttt{FP32}
at $10\%$ and $1\%$ \gls{BLER}, respectively, while outperforming
LS--LMMSE at both targets. At the same nominal $8\times$ weight
compression, INT4 requires an additional $1.98$\,dB and $2.27$\,dB
relative to FP4, respectively, and trails LS--LMMSE. The nonuniform
...
To examine this gap, we apply the per-channel abs-max normalization in
...
by $-0.07$/$-0.10$\,dB after pruning and remains ahead of LS--LMMSE.
In contrast, pruning adds $0.95$--$1.29$\,dB to INT4, which trails
LS--LMMSE by more than $2$\,dB. Thus, FP4 tolerates either compression
path, whereas pruned INT4 degrades sharply.
...
Table~\ref{tab:complexity} reports analytic costs on the evaluation
graph with 132~\glspl{PRB}, 2~users, and 8 iterations. Each slot
requires $19.22$\,G multiply--accumulate operations (MACs).
Of these, $76\%$ arise from $1{\times}1$ pointwise convolutions, the
dominant GEMM-like kernels targeted by low-bit quantization. We
estimate \glspl{BOP} as
\[
  \mathrm{BOPs}
...
Relative to \texttt{FP32}, W8A8 and W4A8 reduce \glspl{BOP} by
$16\times$ and $33\times$, respectively, and reduce stored weight
footprints by $3.8\times$ and $7.1\times$. The modeled W4A8+P
configuration yields $66\times$ and $8.8\times$ reductions in
\glspl{BOP} and weight footprint, respectively. The radio experiments
evaluate W$b$A8 and pruning separately, whereas the +P rows project
their combined analytic cost.

\begin{comment}
Numeric operating points retained for recovery (log-linear interpolation).
Weight-only 10\%/1\% BLER (dB): FP32 2.30/3.58; FP8 2.29/3.52; INT8 2.24/3.51;
FP4 3.62/4.98; INT4 5.60/7.25. LS--LMMSE 4.14/5.43.
WbA8 10\%/1\%: FP8 2.30/3.53; INT8 2.31/3.52; FP4 3.62/4.97; INT4 5.29/6.92.
